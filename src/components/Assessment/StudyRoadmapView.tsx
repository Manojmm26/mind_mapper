      ...
      <section className="space-y-6">
        ...
        <div className="space-y-6">
          {roadmap.milestones.map((milestone, index) => (
            <div
              key={milestone.id}
              className="relative overflow-hidden rounded-[28px] border border-slate-200/90 bg-white p-6 shadow-sm transition-all hover:shadow-md sm:p-8"
            >
              {/* Top Bar */}
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between border-b border-slate-100 pb-4">
                <div className="flex items-center gap-3">
                  <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-2xl bg-indigo-600 text-sm font-black text-white shadow-md shadow-indigo-500/20">
                    {index + 1}
                  </div>
                  <div>
                  ...
                  <div className="rounded-2xl bg-slate-50 p-4">
                    ...
                    <ul className="mt-2 space-y-1.5">
                      {milestone.keyTakeaways.map((takeaway) => (
                        <li key={takeaway} className="flex items-start gap-2 text-xs font-medium text-slate-700">
                  ...
                  <div>
                    ...
                    <div className="grid gap-2.5 sm:grid-cols-2">
                      {milestone.recommendedResources.map((res) => (
                        <div
                          key={res.title}
                          className="flex items-start justify-between gap-2 rounded-xl border border-slate-200/80 bg-white p-3 shadow-2xs hover:border-indigo-300"
                        >
                          <div>
                          ...
                          <span className="shrink-0 rounded-md bg-indigo-50 px-2 py-0.5 text-[9px] font-bold uppercase text-indigo-700">
                            {res.type}
                          </span>
                        </div>
                      ))}
                ...
                {milestone.practiceTask && (
                  <div className="rounded-2xl border border-indigo-100 bg-indigo-50/50 p-4">
                    <div className="flex items-start gap-3">
                      <CheckSquare size={18} className="mt-0.5 shrink-0 text-indigo-600" />
                      <div>
                        <p className="text-[11px] font-black uppercase tracking-wider text-indigo-900">
                          Hands-on Practice Exercise
                        </p>
                        <p className="mt-1 text-xs font-semibold text-indigo-950 leading-5">
                          "{milestone.practiceTask}"
                        </p>
                      </div>
                    </div>
