...
function getAncestorChain(layout: PretextGraphLayout, focusId: string, nodeMap: Map<string, PretextMapNode>) {
...
function buildAtlasScene(
  ...
  const parentNode = layout.parentMap[focusId] ? nodeMap.get(layout.parentMap[focusId] as string) || null : null;
  ...
  const siblingNodes = parentNode
    ...
    : [];
  const ancestorNodes = getAncestorChain(layout, focusId, nodeMap).slice(0, -1).slice(-3);
  ...
  const visibleAncestors = compactScene ? ancestorNodes.slice(-2) : ancestorNodes;
  const visibleSiblings = compactScene ? siblingNodes.slice(0, 2) : siblingNodes;
  ...
  const pushSceneNode = (config: SceneNode) => {
    const position = placeSceneNode(config.node, size, config.targetX, config.targetY, {
  ...
  if (parentNode) {
    pushSceneNode({
      ...
      node: parentNode,
      role: 'parent',
      targetX: compactScene ? centerX - parentNode.width / 2 : centerX - parentNode.width * 0.38 - 290,
  ...
  visibleSiblings.forEach((node, index) => {
    ...
    pushSceneNode({
      ...
      node,
      role: 'sibling',
      targetX: compactScene ? size.width - node.width * 0.72 - 86 + columnOffset : 72 + columnOffset,
  ...
  for (let index = 0; index < childNodes.length; index += itemsPerRing) {
    ...
    ringNodes.forEach((node, ringNodeIndex) => {
      ...
      pushSceneNode({
        ...
        targetScale: ringIndex === 0 ? (compactScene ? 0.78 : 0.86) : compactScene ? 0.66 : 0.72,
...
function placeSceneNode(
  ...
  return {
    x: clamp(targetX, marginX, Math.max(marginX, size.width - node.width - marginX)),
    y: clamp(targetY, marginTop, Math.max(marginTop, size.height - node.height - marginBottom)),
  };
}
...
function drawNodeCard(
  ...
  context.save();
  context.shadowColor = isHovered || isHighlighted ? hexToRgba(palette.accent, 0.4) : palette.shadow;
  ...
  context.strokeStyle = isFocused || isHovered || isHighlighted ? palette.accent : palette.border;
  context.stroke();
  ...
  const descriptionLimit = isFocused ? 3 : role === 'child' ? 2 : role === 'parent' ? 1 : 0;
  if (descriptionLimit > 0 && node.description && node.description.lines.length > 0) {
    cursorY += 12;
  ...
  if ((role === 'child' || role === 'focus') && descendantCount > 0) {
    cursorY += 12;
    const summary = descendantCount === 1 ? '1 connected branch' : `${descendantCount} connected branches`;
    context.fillStyle = hexToRgba('#0f172a', isFocused ? 0.18 : 0.06);
    context.beginPath();
    context.roundRect(paddingX, Math.min(cursorY, height - 36), Math.min(width - paddingX * 2, 144), 22, 11);
    context.fill();
    context.fillStyle = isFocused ? '#e2e8f0' : '#475569';
    context.font = PRETEXT_TYPOGRAPHY.badge.font;
    context.fillText(summary, paddingX + 10, Math.min(cursorY, height - 36) + 5);
  } else if (role === 'focus' && node.tags.length > 0) {
    cursorY += 12;
    drawBadgeRow(context, node.tags, paddingX, Math.min(cursorY, height - 36), 'rgba(255,255,255,0.08)', '#cbd5e1');
  }
...
export function PretextAtlasCanvas({
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      if (focusVisual) {
        const focusCenterX = focusVisual.x + focusVisual.node.width * focusVisual.scale * 0.5;
        const focusCenterY = focusVisual.y + focusVisual.node.height * focusVisual.scale * 0.5;
      ...
      animatedNodes.forEach((visual) => {
        ...
        const startX = focusVisual.x + focusVisual.node.width * focusVisual.scale * 0.5;
        const startY = focusVisual.y + focusVisual.node.height * focusVisual.scale * 0.5;
        const endX = visual.x + visual.node.width * visual.scale * 0.5;
        const endY = visual.y + visual.node.height * visual.scale * 0.5;
        const curve = Math.max(62, Math.abs(endX - startX) * 0.32);
        const palette = getNodePalette(visual.node);

        context.save();
        context.globalAlpha = Math.min(0.8, visual.opacity);
        context.beginPath();
        ...
        context.strokeStyle =
          visual.role === 'parent'
            ...
            : visual.role === 'sibling'
              ...
              : hexToRgba(palette.accent, 0.44);
        context.lineWidth = visual.role === 'child' && visual.id === highlightedChildId ? 2.6 : 1.9;
        context.setLineDash(visual.role === 'sibling' ? [6, 8] : visual.role === 'parent' ? [12, 8] : []);
        context.stroke();
        context.restore();
      });
      ...
      animatedNodes
        ...
        .forEach((visual) => {
          ...
          drawNodeCard(context, visual, hoveredNodeId, highlightedChildId, visitedSet);
        });
    ...
    return (
      getAnimatedNodes(visualsRef.current)
        ...
        .find((visual) => {
          ...
          return localX >= visual.x && localX <= visual.x + width && localY >= visual.y && localY <= visual.y + height;
        }) || null
    );
  };
  ...
  return (
    ...
    >
      ...
      <div className="pointer-events-none absolute left-4 top-4 rounded-2xl border border-white/12 bg-slate-950/30 px-4 py-3 text-[11px] font-semibold tracking-[0.08em] text-slate-200 backdrop-blur-xl">
        Click a signal to move. Arrow up and down changes branches. Enter dives deeper. Backspace returns.
      </div>
              ...
              {highlightedChild ? (
                <>
                  ...
                  <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-300">
                    {highlightedChild.data.description || 'This branch is ready to explore.'}
                  </p>
                </>
              ) : (
                <>
                  <p className="font-display mt-3 text-2xl font-bold tracking-tight text-white">Leaf node reached</p>
                  <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-300">
                    There are no deeper branches from this point. Use Backtrack to move upward or Home to reset the journey.
                  </p>
                </>
              )}
            </div>
      ...
      <div className="pointer-events-none absolute right-4 top-24 hidden max-w-[320px] rounded-2xl border border-white/12 bg-slate-950/30 px-4 py-3 text-left text-[11px] font-semibold tracking-[0.08em] text-slate-300 backdrop-blur-xl xl:block">
        Atlas mode uses the same Pretext-measured cards, but rearranges them around your current focus instead of drawing a static graph.
      </div>
    </div>
  );
}